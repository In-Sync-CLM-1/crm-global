...
export async function replaceVariables(
  ...
  // Caller info — resolved from the user who logged the call/activity
  if (triggerData?.created_by && (result.includes('{{caller_name}}') || result.includes('{{caller_email}}') || result.includes('{{caller_phone}}'))) {
    try {
      ...
      console.error('[TemplateVariables] Error fetching caller profile:', e);
    }
  }
  // Always clear unresolved caller placeholders (in case triggerData.created_by is missing)
  result = result
    .replace(/{{caller_name}}/g, '')
    .replace(/{{caller_email}}/g, '')
    .replace(/{{caller_phone}}/g, '');

  // Demo date/time formatting
  if (triggerData?.demo_date || triggerData?.demo_time) {
    const demoDateRaw: string | null = triggerData.demo_date || null;
    const demoTimeRaw: string | null = triggerData.demo_time || null;
    let demoDateStr = '';
    let demoDayStr = '';
    let demoTimeStr = '';
    ...
    result = result
      .replace(/{{demo_date}}/g, demoDateStr)
      .replace(/{{demo_day}}/g, demoDayStr)
      .replace(/{{demo_time}}/g, demoTimeStr);
  }
  result = result
    .replace(/{{demo_date}}/g, '')
    .replace(/{{demo_day}}/g, '')
