...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session: initialSession } }) => {
      ...
      // Authorize the realtime websocket so RLS policies see auth.uid() during postgres_changes
      supabase.realtime.setAuth(initialSession?.access_token ?? null);
      setIsLoading(false);
    });
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (_event, newSession) => {
        ...
        // Keep the realtime websocket in sync after refresh / sign-in / sign-out
