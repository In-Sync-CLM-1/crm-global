...
export interface InlinePhoneInputHandle {
  /** Returns the current list, appending any typed-but-uncommitted value */
  getFinalPhones: () => PhoneEntry[];
}
...
export const InlinePhoneInput = forwardRef<InlinePhoneInputHandle, InlinePhoneInputProps>(function InlinePhoneInput({ phones, onChange }, ref) {
  const [newPhone, setNewPhone] = useState({ phone: "", phone_type: "mobile" });

  useImperativeHandle(ref, () => ({
    getFinalPhones: () => {
      const trimmed = newPhone.phone.trim();
      if (!trimmed) return phones;
      const pending: PhoneEntry = {
        phone: trimmed,
