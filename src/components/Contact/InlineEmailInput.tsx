...
export const InlineEmailInput = forwardRef<InlineEmailInputHandle, InlineEmailInputProps>(function InlineEmailInput({ emails, onChange }, ref) {
  const [newEmail, setNewEmail] = useState({ email: "", email_type: "work" });

  useImperativeHandle(ref, () => ({
    getFinalEmails: () => {
      const trimmed = newEmail.email.trim();
      if (!trimmed) return emails;
      const pending: EmailEntry = {
        email: trimmed,
