 ...
 export function MessageThread({
   ...
   const scrollToEnd = () => {
     // Double rAF so layout has settled after new bubbles render
   ...
   useEffect(() => {
     if (isLoading || didInitialScrollRef.current || messages.length === 0) return;
   ...
   useEffect(() => {
     ...
     const isNearBottom = v
       ? v.scrollHeight - v.scrollTop - v.clientHeight < 120
       : true;
     const lastMessage = messages[messages.length - 1];
     const isOwnMessage = lastMessage?.sender_id === currentUserId;
     prevCountRef.current = messages.length;
     if (isNearBottom || isOwnMessage) scrollToEnd();
