 ...
 export function ConversationList({
   ...
   const getLastMessagePreview = (conv: Conversation) => {
     ...
     let body: string;
     if (conv.last_message.message_type === "file") body = "📎 Sent a file";
     else if (conv.last_message.message_type === "task_share") body = "📋 Shared a task";
     else body = conv.last_message.content || "";
     if (conv.conversation_type !== "group") return body;
     const senderId = conv.last_message.sender_id;
     let senderLabel: string;
     if (senderId === user?.id) senderLabel = "You";
     else {
       const sender = conv.participants.find((p) => p.user_id === senderId);
       senderLabel = sender?.profile?.full_name?.split(" ")[0] || "Unknown";
     }
