...
interface CalendarViewProps {
  ...
  holidaysByDate?: Record<string, { name: string; is_optional: boolean }>;
          ...
          {days.map((day, idx) => {
            ...
            return (
              <button
                ...
                title={holiday ? `${holiday.name}${holiday.is_optional ? " (optional)" : ""}` : undefined}
                className={cn(
                  ...
                  isCurrentMonth && isWeekend && !holiday && "bg-rose-50/30 dark:bg-rose-950/10",
                  isCurrentMonth && holiday && !holiday.is_optional && "bg-purple-200 dark:bg-purple-900/60 ring-1 ring-purple-400 dark:ring-purple-700",
                  isCurrentMonth && holiday && holiday.is_optional && "bg-purple-100 dark:bg-purple-950/40",
              ...
              >
                <div className="flex flex-col h-full">
                  <div className="flex items-start justify-between gap-1">
                    <span
                      className={cn(
                        "text-sm font-medium w-8 h-8 flex items-center justify-center rounded-full transition-all shrink-0",
                        !isCurrentMonth && "text-muted-foreground/50",
                        isCurrentMonth && !isTodayDate && !isSelected && "group-hover:bg-primary/10",
                        isWeekend && isCurrentMonth && !isTodayDate && !holiday && "text-rose-600 dark:text-rose-400",
                        ...
                        holiday && isCurrentMonth && !isTodayDate && !isSelected && "text-purple-700 dark:text-purple-300 font-bold"
                      )}
                    >
                      {format(day, 'd')}
                    </span>
                    {holiday && isCurrentMonth && (
                      <span className="text-[10px] leading-tight font-semibold text-purple-900 dark:text-purple-100 truncate max-w-[70%]" title={holiday.name}>
